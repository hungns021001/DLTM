...
namespace WebApi.Controllers
{
    ...
    public class CategoryCitiesController : BaseController
    {
        ...
        [HttpGet]
        public async Task<ActionResult<object>> GetApprovedAsync(string keyword, int page = 0, int pageSize = 5)
        {
        ...
        [HttpGet("Get-all")]
        public async Task<ActionResult<List<CategoryCityResponseDto>>> GetAll(StatusEnum statusEnum, string keyword, int page = 0, int pagesize = 5)
        {
        ...
        [HttpGet("Get-active")]
        public async Task<ActionResult<List<CategoryCityResponseDto>>> GetCategoryCityActive(string keyword, int page = 0, int pagesize = 5)
        {
            var newcategorycity = await categoryCityService.GetCategoryCityActive(keyword, page, pagesize);
            var total = await categoryCityService.countAll(StatusEnum.APPROVED, keyword);
            return Ok(new
            {
                Results = newcategorycity,
                Total = total,
            }
            );
        }
        ...
        [HttpGet("Get-Deleted")]
        public async Task<ActionResult<List<CategoryCityResponseDto>>> GetCategoryCityDeleted(string keyword, int page = 0, int pagesize = 5)
        {
            var newcategorycity = await categoryCityService.GetCategoryCityActive(keyword, page, pagesize);
